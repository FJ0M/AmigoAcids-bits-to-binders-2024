...
mkdir -p ${WORKDIR}/RF_diff/att1/normal_weights
...
cd ${RESULTS_DIR}
"${RFdiff_path}" --config-name=symmetry inference.symmetry="C2" \ 
    'potentials.guiding_potentials=["type:olig_contacts,weight_intra:1,weight_inter:0.1"]' \
    potentials.olig_intra_all=True potentials.olig_inter_all=True potentials.guide_scale=2.0 potentials.guide_decay="quadratic" \
    inference.output_prefix="${RESULTS_DIR}" inference.input_pdb="${INPUT_PDB}" \
    inference.ckpt_override_path="${RFdiff_beta_model_path}" \
    'contigmap.contigs=[C140-190/0  80]' \
